...
(function() {
    ...
    let bgColors = ["black", "white", "rgba(0,255,155,1)",  "silver"],
    ...
    function getResult(theta) {
        ...
        let arcSecs = (Math.PI*2)/strafen.length,
            angle = theta + Math.PI/2 - arcSecs/2,
            arc = angle%(Math.PI*2),
            idx = Math.ceil(arc/arcSecs);

        console.log(arc, arcSecs, arc/arcSecs);
        console.log(strafen[strafen.length-idx]);
        strafen.splice(strafen.length-idx, 1);
    ...
    function drawWheel(rotation) {
        ...
        strafen.forEach(function(strafe,i){
            
            let arcSec = 2*(Math.PI)/strafen.length

            // Arcs
            ctx.beginPath();
            ctx.arc(0,0, od+10, -arcSec/2, arcSec/2);
            ctx.lineTo(0,0);
            ...
            ctx.beginPath();
            //ctx.moveTo(0,0);
            ctx.arc(0,0, od, -arcSec/2, arcSec/2);
            ctx.lineTo(0,0);
        ...
        //Inner thing
        ctx.beginPath();
        ctx.arc(board.cpx, board.cpy, td - 30, 0, Math.PI*2);
